...
INC_DIRS = inc ../../Lib/inc ../../$(PLATFORM)/CM3 ../../$(PLATFORM)/inc
...
SRC_DIRS =  src ../../Lib/src ../../$(PLATFORM)/CM3/startup ../../$(PLATFORM)/CM3 ../../$(PLATFORM)/src
...
CXX_SRCS = $(wildcard $(patsubst %, %/*.$(CXX_EXT), . $(SRC_DIRS)))
...
C_SRCS = $(wildcard $(patsubst %, %/*.$(C_EXT), . $(SRC_DIRS)))
...
# current folder and SRC_DIRS folders will be compiled and linked)
AS_SRCS = $(wildcard $(patsubst %, %/*.$(AS_EXT), . $(SRC_DIRS)))
...
C_WARNINGS = -Wall -Wstrict-prototypes -Wextra -Wno-pointer-sign -Wno-builtin-declaration-mismatch -Wno-discarded-qualifiers
...
ifeq ($(strip $(IMAGE_DIR)), )
	IMAGE_DIR_F =
else
	IMAGE_DIR_F = $(strip $(IMAGE_DIR))/
endif
...
# flags for linker
LD_FLAGS = -T$(LD_SCRIPT) -g -specs=nano.specs -Wl,-Map=$(IMAGE_DIR_F)$(PROJECT).map,--cref,--no-warn-mismatch
...
ELF = $(IMAGE_DIR_F)$(PROJECT).elf
HEX = $(IMAGE_DIR_F)$(PROJECT).hex
BIN = $(IMAGE_DIR_F)$(PROJECT).bin
LSS = $(IMAGE_DIR_F)$(PROJECT).lss
DMP = $(IMAGE_DIR_F)$(PROJECT).dmp
...
IMAGE_GENERATED = $(wildcard $(patsubst %, $(IMAGE_DIR)*.%, bin d dmp elf hex lss lst map o))
GENERATED = $(wildcard $(patsubst %, $(OUT_DIR)*.%, bin d dmp elf hex lss lst map o))
...
clean: 
	-rm -f $(IMAGE_DIR)/*.*
	-rm -f $(OUT_DIR)/*.*
#ifeq ($(strip $(IMAGE_DIR_F)), )
#	@echo 'Removing all generated output files'
#else
#	@echo 'Removing all generated output files from image directory: $(IMAGE_DIR_F)'
#endif
#ifneq ($(strip $(IMAGE_GENERATED)), )
#	$(RM) $(IMAGE_GENERATED)
#else
